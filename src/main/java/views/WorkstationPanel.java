...
public class WorkstationPanel extends JPanel {
    ...
    private final JLabel lblBranch = new JLabel(Props.getLangProperty("MainFrame.notSet"));
    private final JLabel lblCounter = new JLabel(Props.getLangProperty("MainFrame.notSet"));
    private final JLabel lblWorkProfile = new JLabel(Props.getLangProperty("MainFrame.notSet"));
                ...
                try {
                    ...
                    if (callFowards) {
                        ...
                        try {
                            ...
                            log.error("Failed to data", e);
                            mv.showMessageDialog(Props.getLangProperty("MainFrame.errorCounterNotSet"),
                                    JOptionPane.INFORMATION_MESSAGE);
                        }
                    }
    ...
    private class Flash implements Runnable {
        public void run() {
            while (true) {
                ...
                if (flashAmountCurrent >= 10) {
                    timer.stop();
                    lblA.setForeground(Color.BLACK);
                    flashAmountCurrent = 0;
                    flash = false;
                    startedFlash = false;
                }
            }
